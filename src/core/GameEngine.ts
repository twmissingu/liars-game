...
export class GameEngine {
  ...
  aiPlayCards(aiId: 'ai' | 'ai2' | 'ai3'): GameState {
    ...
    if (liarCards.length > 0 && Math.random() > 0.3) {
      ...
      const count = Math.max(
        1,
        Math.min(maxAiCards, liarCards.length, Math.floor(Math.random() * maxAiCards) + 1)
      );
      ...
      const count = Math.max(
        1,
        Math.min(maxAiCards, otherCards.length, Math.floor(Math.random() * maxAiCards) + 1)
      );
  ...
  private resolveChallenge(challenger: 'player' | 'ai' | 'ai2' | 'ai3'): GameState {
    ...
    // 确定谁受伤（撒谎者或质疑失败者）
    const loser: 'player' | 'ai' | 'ai2' | 'ai3' = wasLie ? playedCards.playerId : challenger;

    // 获取玩家名称
    const getPlayerName = (id: string) => {
      if (id === 'player')
        return this.playerCharacter ? getCharacterName(this.playerCharacter) : '玩家';
    ...
    if (wasLie) {
      ...
      if (
        playedBy === 'player' &&
        this.playerCharacter === 'kallen' &&
        this.state.playerStats.kallenBoostActive
      ) {
